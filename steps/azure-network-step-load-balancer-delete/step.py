...
print('Deleting {} Azure Load Balancer(s)'.format(len(resource_ids)))
handle_list = []
for resource_id in resource_ids:
  resource_group_name = resource_id.split('/')[4] # Resource group name
  name = resource_id.split('/')[8] 
  print(resource_id)
  async_operation = network_client.load_balancers.begin_delete(resource_group_name, name)
...
print('\nAll specified Load Balancers are deleted')
